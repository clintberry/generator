...
// Run starts the config generator
func Run(templateDir string, outputDir string, data interface{}) error {
	if _, err := os.Stat(outputDir); os.IsNotExist(err) {
		fmt.Println("Output directory doesn't exit. Creating folder structure...")
		err := os.MkdirAll(outputDir, 0777)
		if err != nil {
			return err
		}
	}
	//applicationPath := filepath.Join(outputDir, data.Config.Name+"-"+clustername)
	applicationPath := outputDir
	// if _, err := os.Stat(applicationPath); os.IsNotExist(err) {
	// 	fmt.Println("Application doesn't exist, creating application manifest folder")

	// 	//cmd := exec.Command("ks", "init", data.Config.Name, "--dir="+applicationPath)
	// 	//err = cmd.Run()
	// 	// if err != nil {
	// 	// 	panic(err)
	// 	// }
	...
	err = os.MkdirAll(applicationPath, 0777)
	if err != nil {
		return err
	}
	...
	err = ExecuteTemplates(tmpls, applicationPath, data)
	if err != nil {
		return err
	}
	return nil
...
func ExecuteTemplates(t *Template, outputDir string, data interface{}) error {
	fmt.Println("Executing templates in dir '" + outputDir + "'")
